...
public interface CategoryTransactionCount {
    ...
    @Query("SELECT COUNT(DISTINCT t.category) FROM Transaction t WHERE t.createdAt BETWEEN :start AND :end")
    Long countActiveCategoriesThisWeek(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end);
