...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    Long countTransactionsByDateRange(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end);
    
    @Query("SELECT DATE(t.createdAt) as date, SUM(t.amount) as total, COUNT(t) as count " +
           "FROM Transaction t WHERE DATE(t.createdAt) >= :fromDate " +
           "GROUP BY DATE(t.createdAt) ORDER BY date DESC")
    List<Object[]> findWeeklySpendingData(@Param("fromDate") LocalDate fromDate);
